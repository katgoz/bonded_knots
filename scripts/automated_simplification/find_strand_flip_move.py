"""
Implements detection and execution of strand-flip moves in a planar diagram.

A strand-flip move identifies a strand that passes exclusively through overcrossings or undercrossings and determines
whether it can be rerouted along an alternative path with fewer crossings. If such a rerouting exists, the strand is replaced accordingly.

This module is part of the project described in "Classification of Bonded Knots" (see classification_of_bonded_knots.pdf).

The general concept of the strand-flip move, together with illustrative examples, 
is described in Section 5.1.3, "Diagram-Reducing Strand Flip Move", of the accompanying manuscript.

Some of the implemented functions are also reused in:
- Section 5.1.1: Identifying the inner vertex closest to the outer face
- Section 5.1.2: Moving the Inner Vertex onto the Outer Face

of "Classification of Bonded Knots" (see classification_of_bonded_knots.pdf).
"""
...
def find_path_of_type(diagram, start, end=None, allowed_type={"over"}):
    ...
    If end is None, it follows the strand as long as possible and returns the maximal path.


    This procedure is described under the heading "Finding a Strand Passing Exclusively Over Crossings"
    in the accompanying manuscript 'Classification of Bonded Knots'.
...
def min_crossings_path(diagram, start_endpoints, target_endpoints, blocked_transitions):
    ...
    Other transitions through the same crossing remain allowed. These blocked transitions are provided externally as preprocessing data.
    
    PATH RECONSTRUCTION:
    The state space includes a neutral orientation "A", meaning that no left/right decision has been made yet. 
    Once a first turn is taken, the orientation becomes fixed to "L" or "R".
    If a path remains in state "A", it is interpreted as symmetric and resolved as pure 'L' path at reconstruction time.

    In this model we effectively traverse only one representative endpoint from each twin pair during propagation;
    therefore both representatives are seeded initially to avoid losing valid traversal branches.

    The missing twin states are later reconstructed during the final path recovery.


    This procedure is described under the heading "Finding a Minimal-Crossing Path" in the accompanying manuscript 'Classification of Bonded Knots'.
...
def min_crossings_path_all(diagram, start_endpoints, target_endpoints, blocked_transitions):
    """
    This function is an extension of the single-target minimal crossing path algorithm defined above (min_crossings_path).
    Instead of stopping at the first reached target, it continues the search and returns minimal-cost paths to all target endpoints simultaneously.

    For each target, endpoints are grouped into equivalence classes consisting of the target and its twin endpoint, 
    and the algorithm stores the minimal cost state per target equivalence class.

    The final output is a list of minimal-cost paths, one for each reachable target class.
    ...
    Args:
        ...
        target_endpoints (Iterable[Endpoint]):
            Endpoints at which the search is evaluated. The algorithm computes minimal-cost paths to all reachable targets simultaneously.
    ...
    Returns:
        list[tuple[float, list[tuple[Endpoint, str]]]] | None:
            A list of (minimal_cost, reconstructed_path_states) pairs, one for each reachable target equivalence class (target + twin), where each
            state is (endpoint, last_turn).

            minimal_cost represents the number of crossings that would be created by inserting an edge alongside the reconstructed path
             and is minimal among all possible paths to that target class.
...
def max_between_nodes(diagram, node1, node2, allowed_type):
    ...
    This procedure corresponds to the first step under the heading "Algorithm: Detection of Diagram-Reducing Strand Flip Move"
    in the accompanying manuscript 'Classification of Bonded Knots'.
...
def min_between_nodes(diagram, max_path):
    ...
    This procedure corresponds to the second step under the heading "Algorithm: Detection of Diagram-Reducing Strand Flip Move"
    in the accompanying manuscript 'Classification of Bonded Knots'.
...
def find_strand_flip_move(diagram, allowed_type):
    ...
    This procedure corresponds to the algorithm described under the heading "Algorithm: Detection of Diagram-Reducing Strand Flip Move"
    in the accompanying manuscript 'Classification of Bonded Knots'.
...
def detect_strand_flip_moves_from_file(filename):
    """
    Applies strand flip detection to multiple planar diagrams loaded from a file.

    This function serves as a pipeline over a collection of diagrams. For each diagram,
    it checks both "over" and "under" strand flip moves using find_strand_flip_move.
    If a valid reduction is found, the diagram is recorded as a positive case.

    Args:
        filename:
            Path to a file containing PD notation strings, one per line.

    Returns:
        None

    Side effects:
        Prints all diagrams for which a strand flip move is detected and reports their count.
    """
...
def insert_path_core(diagram, alternative_path, kind , max_path = None, outer = False, alt=False):
    ...
    For each step, the function checks whether a crossing must be inserted between consecutive elements of the path.
    If so, a new crossing is created via `insert_crossing` function, where the `from_ep` argument is used to connect
    ...
    The construction does not explicitly close the strand; it only builds its embedding and returns its endpoints.

    If `max_path` is provided, crossings involving segments belonging to the maximal strand are skipped
    when they are expected to be removed later in the algorithm.

    If `outer=True`, the function tracks whether the constructed strand enters the outer face and records all endpoints belonging to it.

    If `alt=True`, the function returns a second sequence of (endpoint, turn) steps that describes how to build a parallel strand 
    alongside the one being inserted. This sequence can be used later to construct that parallel path in the diagram.

    This procedure corresponds to the second step illustrated in Figure 5.3 ("Node externalization along a selected path").
    It is used with `alt=True` (since three strands are inserted) and also forms part of the "Diagram-Reducing Strand Flip Move"
    (with `alt=False` used as a single-path insertion case) described in the accompanying manuscript "Classification of Bonded Knots".

    Args:
        diagram : PlanarDiagram
            Diagram being modified in-place.
        alternative_path : list[tuple[Endpoint, str]]
            Sequence of (endpoint, turn) instructions defining the strand.
        kind : str
            Specifies whether inserted crossings correspond to "over" or "under".
        max_path : list, optional
            Reference path used to avoid inserting crossings that will be removed.
        outer : bool
            If True, tracks endpoints belonging to the outer face.
        alt : bool
            If True, additionally returns a parallel alternative path.

    Returns:
        tuple:
            start_ep, free_ep
            Optionally extended with:
                new_outer (if outer=True)
                new_alt_path (if alt=True)

            start_ep:
                first endpoint of the constructed strand
            free_ep:
                last endpoint of the constructed strand
            new_outer:
                list of endpoints belonging to the outer face (only if outer=True)
            new_alt_path:
                sequence of (endpoint, turn) describing a parallel strand (only if alt=True)
    """
...
def attach_ends(diagram, path, max_path, start_ep, free_ep, path_end1, path_end2, start = False):
    ...
    This procedure is used after inserting path core of strands in both  "Moving the Inner Vertex onto the Outer Face"
    and "Diagram-Reducing Strand Flip Move" described in the accompanying manuscript "Classification of Bonded Knots".
    ...
    path_end1, path_end2 : Endpoint or None
        Optional overrides for the computed attachment points.
        If provided, they replace the standard vertex/crossing-based computation at the corresponding end
        (used when an endpoint was previously a crossing that got removed due to a loop).
    start : bool, optional
...
def delete_path(diagram, max_path):
    ...
    The function also computes boundary attachment points for reconnection. If the first or
    last endpoint of `max_path` lies inside a removable region, the function follows twin
    connections until reaching a stable endpoint outside the removed region. These endpoints
    are returned as `path_end1` and `path_end2` and are used later for correct reconnection.

    The diagram is modified in-place.

    Note:
        During traversal, a cyclic loop (unknot-like situation) may be detected, but it is
        not handled in this function and is resolved in a later stage of the pipeline.
    ...
    Returns:
        tuple[Endpoint or None, Endpoint or None]:
            (path_end1, path_end2) boundary endpoints used for reconnection logic.
            Either value may be None if no adjustment is required.
...
def perform_strand_flip(diagram, max_path, alternative_path, kind, outer = False):
    ...
    (1) construction of the new strand core via `insert_path_core`,
    (2) removal of the old strand structure via `delete_path`,
    (3) reconnection of the new strand into the diagram via `attach_ends`.

    If `outer=True`, the function additionally tracks endpoints that belong to the outer face
    during construction of the new strand and returns this information.

    After the flip, the function ensures that the number of connected components is preserved
    by inserting unknots if necessary, since removal of a strand belonging to an unknot
    component may eliminate that component during `delete_path`.
...
if __name__ == "__main__":
    pd_text = "V[0,1,2],V[3,4,5],V[6,7,4],V[0,8,9],X[1,10,3,11],X[11,5,7,12],X[9,13,6,10],X[2,12,13,8]"
    D = kp.from_pd_notation(pd_text)
    print("BEFORE STRAND FLIP:", D)
    find_and_perform_strand_flip(D)
    print("AFTER STRAND FLIP:", D)
